...
void setup() {
  Serial.begin(115200);
  Serial.println("DHT ESP32 is Start now !");
  Serial.println();
...
void loop() {
  ...
  Serial.println(" tmp$" + String(newValues.temperature) + " hud$" + String(newValues.humidity));
  delay(200);
    ...
    if (microgear.connected()) {
        ...
        if (timer >= 1000) {
            ...
            microgear.chat( TARGET,tmp+(newValues.temperature));
            microgear.chat( TARGET,hud+(newValues.humidity));
